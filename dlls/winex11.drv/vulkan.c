...
static CRITICAL_SECTION_DEBUG critsect_debug = {
    ...
    0,
    0,
    {(DWORD_PTR)(__FILE__ ": context_section")}};
...
struct wine_vk_surface {
  ...
  LONG swapchain_count; /* surface can have one active an many retired
                           swapchains */
  HDC hdc;
  HWND hwnd;
...
static VkResult (*pvkCreateInstance)(const VkInstanceCreateInfo *,
                                     const VkAllocationCallbacks *,
                                     VkInstance *);
static VkResult (*pvkCreateSwapchainKHR)(VkDevice,
                                         const VkSwapchainCreateInfoKHR *,
                                         const VkAllocationCallbacks *,
                                         VkSwapchainKHR *);
static VkResult (*pvkCreateXlibSurfaceKHR)(VkInstance,
                                           const VkXlibSurfaceCreateInfoKHR *,
                                           const VkAllocationCallbacks *,
                                           VkSurfaceKHR *);
static void (*pvkDestroyInstance)(VkInstance, const VkAllocationCallbacks *);
static void (*pvkDestroySurfaceKHR)(VkInstance, VkSurfaceKHR,
                                    const VkAllocationCallbacks *);
static void (*pvkDestroySwapchainKHR)(VkDevice, VkSwapchainKHR,
                                      const VkAllocationCallbacks *);
static VkResult (*pvkEnumerateInstanceExtensionProperties)(
    const char *, uint32_t *, VkExtensionProperties *);
static VkResult (*pvkGetDeviceGroupSurfacePresentModesKHR)(
    VkDevice, VkSurfaceKHR, VkDeviceGroupPresentModeFlagsKHR *);
static void *(*pvkGetDeviceProcAddr)(VkDevice, const char *);
static void *(*pvkGetInstanceProcAddr)(VkInstance, const char *);
static VkResult (*pvkGetPhysicalDevicePresentRectanglesKHR)(VkPhysicalDevice,
                                                            VkSurfaceKHR,
                                                            uint32_t *,
                                                            VkRect2D *);
static VkResult (*pvkGetPhysicalDeviceSurfaceCapabilities2KHR)(
    VkPhysicalDevice, const VkPhysicalDeviceSurfaceInfo2KHR *,
    VkSurfaceCapabilities2KHR *);
static VkResult (*pvkGetPhysicalDeviceSurfaceCapabilitiesKHR)(
    VkPhysicalDevice, VkSurfaceKHR, VkSurfaceCapabilitiesKHR *);
static VkResult (*pvkGetPhysicalDeviceSurfaceFormats2KHR)(
    VkPhysicalDevice, const VkPhysicalDeviceSurfaceInfo2KHR *, uint32_t *,
    VkSurfaceFormat2KHR *);
static VkResult (*pvkGetPhysicalDeviceSurfaceFormatsKHR)(VkPhysicalDevice,
                                                         VkSurfaceKHR,
                                                         uint32_t *,
                                                         VkSurfaceFormatKHR *);
static VkResult (*pvkGetPhysicalDeviceSurfacePresentModesKHR)(
    VkPhysicalDevice, VkSurfaceKHR, uint32_t *, VkPresentModeKHR *);
static VkResult (*pvkGetPhysicalDeviceSurfaceSupportKHR)(VkPhysicalDevice,
                                                         uint32_t, VkSurfaceKHR,
                                                         VkBool32 *);
static VkBool32 (*pvkGetPhysicalDeviceXlibPresentationSupportKHR)(
    VkPhysicalDevice, uint32_t, Display *, VisualID);
static VkResult (*pvkGetSwapchainImagesKHR)(VkDevice, VkSwapchainKHR,
                                            uint32_t *, VkImage *);
static VkResult (*pvkQueuePresentKHR)(VkQueue, const VkPresentInfoKHR *);
static VkResult (*pvkWaitForFences)(VkDevice device, uint32_t fenceCount,
                                    const VkFence *pFences, VkBool32 waitAll,
                                    uint64_t timeout);
static VkResult (*pvkCreateFence)(VkDevice device,
                                  const VkFenceCreateInfo *pCreateInfo,
                                  const VkAllocationCallbacks *pAllocator,
                                  VkFence *pFence);
static void (*pvkDestroyFence)(VkDevice device, VkFence fence,
                               const VkAllocationCallbacks *pAllocator);

static void *X11DRV_get_vk_device_proc_addr(const char *name);
static void *X11DRV_get_vk_instance_proc_addr(VkInstance instance,
                                              const char *name);
 ...
 * VkInstanceCreateInfo. Caller is responsible for allocation and cleanup of
 * 'dst'.
 */
static VkResult
wine_vk_instance_convert_create_info(const VkInstanceCreateInfo *src,
  ...
  if (src->enabledExtensionCount > 0) {
    enabled_extensions = heap_calloc(src->enabledExtensionCount,
...
wine_vk_surface_grab(struct wine_vk_surface *surface) {
...
void destroy_vk_surface(HWND hwnd) {
  ...
  EnterCriticalSection(&context_section);
  LIST_FOR_EACH_ENTRY_SAFE(surface, next, &surface_list, struct wine_vk_surface,
                           entry) {
...
static BOOL wine_vk_surface_set_offscreen(struct wine_vk_surface *surface,
                                          BOOL offscreen) {
#ifdef SONAME_LIBXCOMPOSITE
  if (usexcomposite) {
    if (!surface->offscreen && offscreen) {
      FIXME("Redirecting vulkan surface offscreen, expect degraded "
            "performance.\n");
      pXCompositeRedirectWindow(gdi_display, surface->window,
                                CompositeRedirectManual);
    } else if (surface->offscreen && !offscreen) {
      FIXME("Putting vulkan surface back onscreen, expect standard "
            "performance.\n");
      pXCompositeUnredirectWindow(gdi_display, surface->window,
                                  CompositeRedirectManual);
    }
  ...
  if (offscreen)
    FIXME("Application requires child window rendering, which is not "
          "implemented yet!\n");
  surface->offscreen = offscreen;
  return !offscreen;
}

void resize_vk_surfaces(HWND hwnd, Window active, int mask,
...
void sync_vk_surface(HWND hwnd, BOOL known_child) {
  ...
  LIST_FOR_EACH_ENTRY(surface, &surface_list, struct wine_vk_surface, entry) {
    if (surface->hwnd != hwnd)
      continue;
    if (surface->swapchain_count)
      surface_with_swapchain_count++;
    surface->known_child = known_child;
  }
  TRACE("hwnd %p surface_with_swapchain_count %u known_child %u\n", hwnd,
        surface_with_swapchain_count, known_child);
  LIST_FOR_EACH_ENTRY(surface, &surface_list, struct wine_vk_surface, entry) {
    if (surface->hwnd != hwnd)
      continue;
    if (surface_with_swapchain_count > 1)
      wine_vk_surface_set_offscreen(surface, TRUE);
    else
...
Window wine_vk_active_surface(HWND hwnd) {
  ...
  if (!active)
    ...
    TRACE("hwnd %p surface_with_swapchain_count %u known_child %u\n", hwnd,
          surface_with_swapchain_count, active->known_child);
    if (surface_with_swapchain_count > 1)
      wine_vk_surface_set_offscreen(active, TRUE);
    else
      wine_vk_surface_set_offscreen(active, active->known_child);
...
static VkResult X11DRV_create_vk_instance_with_callback(
    const VkInstanceCreateInfo *create_info,
    const VkAllocationCallbacks *allocator, VkInstance *instance,
    VkResult(WINAPI *native_vkCreateInstance)(
  ...
  VkResult res;
  TRACE("create_info %p, allocator %p, instance %p, native_vkCreateInstance "
        "%p, context %p.\n",
        create_info, allocator, instance, native_vkCreateInstance,
        native_vkCreateInstance_context);
  ...
  if (native_vkCreateInstance)
    res = native_vkCreateInstance(&create_info_host, NULL /* allocator */,
                                  instance, pvkGetInstanceProcAddr,
                                  native_vkCreateInstance_context);
  else
...
static VkResult
X11DRV_vkAcquireNextImageKHR(VkDevice device, VkSwapchainKHR swapchain,
  ...
  EnterCriticalSection(&context_section);
  if (!XFindContext(gdi_display, (XID)swapchain, vulkan_swapchain_context,
                    (char **)&surface)) {
  ...
  result = pvkAcquireNextImageKHR(device, swapchain, timeout, semaphore, fence,
                                  image_index);
  if (result == VK_SUCCESS && hdc && surface && surface->offscreen) {
    if (wait_fence)
      pvkWaitForFences(device, 1, &fence, 0, timeout);
    ...
    if (surface->present_mode == VK_PRESENT_MODE_MAILBOX_KHR)
      if (once++)
        FIXME("Application requires child window rendering with mailbox "
              "present mode, expect possible tearing!\n");
  }

  if (fence != orig_fence)
    pvkDestroyFence(device, fence, NULL);
  if (surface)
    wine_vk_surface_release(surface);
  return result;
}
...
static VkResult
X11DRV_vkAcquireNextImage2KHR(VkDevice device,
  ...
  return X11DRV_vkAcquireNextImageKHR(
...
static VkResult X11DRV_vkCreateInstance(const VkInstanceCreateInfo *create_info,
                                        ...
                                        VkInstance *instance) {
  return X11DRV_create_vk_instance_with_callback(create_info, allocator,
                                                 instance, NULL, NULL);
}

static VkResult X11DRV_vkCreateSwapchainKHR(
  ...
  /* force fifo when running offscreen so the acquire fence is more likely to be
   * vsynced */
  if (x11_surface->offscreen &&
  ...
  LIST_FOR_EACH_ENTRY(other, &surface_list, struct wine_vk_surface, entry) {
    if (other->hwnd != x11_surface->hwnd)
      continue;
    if (!other->swapchain_count)
      continue;
  ...
  result = pvkCreateSwapchainKHR(device, &create_info_host,
...
static VkResult X11DRV_vkCreateWin32SurfaceKHR(
  ...
  if (x11_surface->hwnd) {
    ...
    x11_surface->hwnd_thread_id =
        GetWindowThreadProcessId(x11_surface->hwnd, NULL);
  } else {
  ...
  if (create_info->hwnd &&
      (GetWindow(create_info->hwnd, GW_CHILD) ||
  ...
  res = pvkCreateXlibSurfaceKHR(instance, &create_info_host,
                                NULL /* allocator */, &x11_surface->surface);
  if (res != VK_SUCCESS) {
...
static void X11DRV_vkDestroyInstance(VkInstance instance,
                                     const VkAllocationCallbacks *allocator) {
  TRACE("%p %p\n", instance, allocator);
...
static void X11DRV_vkDestroySurfaceKHR(VkInstance instance,
...
static void
X11DRV_vkDestroySwapchainKHR(VkDevice device, VkSwapchainKHR swapchain,
                             const VkAllocationCallbacks *allocator) {
  struct wine_vk_surface *surface;
...
static VkResult X11DRV_vkEnumerateInstanceExtensionProperties(
  ...
  /* We will return the same number of instance extensions reported by the host
   * back to winevulkan. Along the way we may replace xlib extensions with their
   * win32 equivalents. Winevulkan will perform more detailed filtering as it
   * knows whether it has thunks for a particular extension.
   */
  ...
  for (i = 0; i < *count; i++) {
    /* For now the only x11 extension we need to fixup. Long-term we may need an
     * array. */
...
static VkResult X11DRV_vkGetDeviceGroupSurfacePresentModesKHR(
  ...
  return pvkGetDeviceGroupSurfacePresentModesKHR(device, x11_surface->surface,
                                                 flags);
}
...
static void *X11DRV_vkGetInstanceProcAddr(VkInstance instance,
                                          const char *name) {
  void *proc_addr;
...
static VkResult X11DRV_vkGetPhysicalDevicePresentRectanglesKHR(
    VkPhysicalDevice phys_dev, VkSurfaceKHR surface, uint32_t *count,
    VkRect2D *rects) {
  struct wine_vk_surface *x11_surface = surface_from_handle(surface);
  ...
  return pvkGetPhysicalDevicePresentRectanglesKHR(
      phys_dev, x11_surface->surface, count, rects);
}
...
static VkResult X11DRV_vkGetPhysicalDeviceSurfaceCapabilities2KHR(
  ...
  surface_info_host.surface =
      surface_from_handle(surface_info->surface)->surface;

  if (pvkGetPhysicalDeviceSurfaceCapabilities2KHR)
    return pvkGetPhysicalDeviceSurfaceCapabilities2KHR(
        phys_dev, &surface_info_host, capabilities);
  ...
  return pvkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      phys_dev, surface_info_host.surface, &capabilities->surfaceCapabilities);
}

static VkResult X11DRV_vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
    VkPhysicalDevice phys_dev, VkSurfaceKHR surface,
  ...
  return pvkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      phys_dev, x11_surface->surface, capabilities);
}

static VkResult X11DRV_vkGetPhysicalDeviceSurfaceFormats2KHR(
    VkPhysicalDevice phys_dev,
  ...
  if (!formats)
    return pvkGetPhysicalDeviceSurfaceFormatsKHR(
        phys_dev, surface_info_host.surface, count, NULL);

  formats_host = heap_calloc(*count, sizeof(*formats_host));
  if (!formats_host)
    return VK_ERROR_OUT_OF_HOST_MEMORY;
  result = pvkGetPhysicalDeviceSurfaceFormatsKHR(
      phys_dev, surface_info_host.surface, count, formats_host);
...
static VkResult X11DRV_vkGetPhysicalDeviceSurfaceFormatsKHR(
    VkPhysicalDevice phys_dev, VkSurfaceKHR surface, uint32_t *count,
...
static VkResult X11DRV_vkGetPhysicalDeviceSurfacePresentModesKHR(
    VkPhysicalDevice phys_dev, VkSurfaceKHR surface, uint32_t *count,
  ...
  return pvkGetPhysicalDeviceSurfacePresentModesKHR(
      phys_dev, x11_surface->surface, count, modes);
}

static VkResult X11DRV_vkGetPhysicalDeviceSurfaceSupportKHR(
    VkPhysicalDevice phys_dev, uint32_t index, VkSurfaceKHR surface,
    VkBool32 *supported) {
  struct wine_vk_surface *x11_surface = surface_from_handle(surface);

  TRACE("%p, %u, 0x%s, %p\n", phys_dev, index, wine_dbgstr_longlong(surface),
        supported);

  return pvkGetPhysicalDeviceSurfaceSupportKHR(phys_dev, index,
                                               x11_surface->surface, supported);
}
...
X11DRV_vkGetPhysicalDeviceWin32PresentationSupportKHR(VkPhysicalDevice phys_dev,
  ...
  return pvkGetPhysicalDeviceXlibPresentationSupportKHR(
      phys_dev, index, gdi_display, default_visual.visual->visualid);
}
...
static VkResult X11DRV_vkGetSwapchainImagesKHR(VkDevice device,
                                               ...
                                               uint32_t *count,
                                               VkImage *images) {
  TRACE("%p, 0x%s %p %p\n", device, wine_dbgstr_longlong(swapchain), count,
        images);
  return pvkGetSwapchainImagesKHR(device, swapchain, count, images);
}

static VkResult X11DRV_vkQueuePresentKHR(VkQueue queue,
...
static VkBool32 X11DRV_query_fs_hack(VkSurfaceKHR surface, VkExtent2D *real_sz,
  ...
  if (XFindContext(gdi_display, x11_surface->window, winContext,
                   (char **)&hwnd) != 0) {
  ...
  if (fs_hack_enabled(monitor) && !x11_surface->offscreen) {
    ...
    TRACE("real_rect:%s user_rect:%s scaled:%dx%d scaled_origin:%s\n",
          wine_dbgstr_rect(&real_rect), wine_dbgstr_rect(&user_rect), scaled.cx,
          scaled.cy, wine_dbgstr_point(&scaled_origin));
